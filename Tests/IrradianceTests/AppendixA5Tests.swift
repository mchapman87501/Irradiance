...
class AppendixA5Tests: XCTestCase {
    ...
    func testIrradianceConditions() throws {
        // Again, all expected values are from Solar Position Algorithm For Solar Radiation Applications,
        // NREL 2008, Appendix A.5.
        ...
        let eGeoLon = 204.0182616917
        eq(info.geoPos.longitude, eGeoLon, "Geo Lon")
        let eGeoLat = 0.0001011219
        eq(info.geoPos.latitude, eGeoLat, "Geo Lat")

        let eApparentSunLon = 204.0085519281

        eq(info.apparentSunLon, eApparentSunLon, "Apparent Sun Longitude")

        let eGeoSunRightAscension = 202.22741
        eq(info.geoSunAngles.rightAscension, eGeoSunRightAscension, "Geocentric Sun Right Ascension")
        let eGeoSunDeclination = -9.31434
        eq(info.geoSunAngles.declination, eGeoSunDeclination, "Geocentric Sun Declination")

        let eObserverLocalHourAngle = 11.105900
        eq(info.topoAngles.observerLocalHourAngle, eObserverLocalHourAngle, "Observer Local Hour Angle")

        let eTopoLocalHourAngle = 11.10629
        eq(info.topoAngles.localHourAngle, eTopoLocalHourAngle, "Topocentric Local Hour Angle")

        let eTopoSunRightAscension = 202.22704
        eq(info.topoAngles.rightAscension, eTopoSunRightAscension, "Topocentric Sun Right Ascension")

        let eTopoSunDeclination = -9.316179
        eq(info.topoAngles.declination, eTopoSunDeclination, "Topocentric Sun Declination")

        let eTopoZenithAngle = 50.11162
        eq(info.topoAngles.zenithAngle, eTopoZenithAngle, "Topocentric Zenith Angle")

        let eTopoAzimuthAngle = 194.34024
        eq(info.topoAngles.azimuthAngle, eTopoAzimuthAngle, "Topocentric Azimuth Angle")

        let eIncidenceAngle = 25.18700
        eq(info.incidenceAngle, eIncidenceAngle, "Incidence Angle")

        // TBD Sun's mean longitude, equation of time, transit, sunrise, sunset
    }
